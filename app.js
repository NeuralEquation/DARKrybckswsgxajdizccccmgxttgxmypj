(function () {
  ...
  function handleStandaloneWaitImport() {
    const url = WAIT_IMPORT_CONFIG.disneyRealUrl;
    if (!url) {
      updateWaitImportStatus("danger", "URLを開けません");
      return;
    }
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(url).then(function () {
        updateWaitImportStatus("danger", "PWAでは使えません。通常ブラウザで開くURLをコピーしました");
      }).catch(function () {
        updateWaitImportStatus("danger", "PWAでは使えません。通常ブラウザで開いてください");
      });
      return;
    }
    updateWaitImportStatus("danger", "PWAでは使えません。通常ブラウザで開いてください");
